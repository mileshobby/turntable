...
var Square = function () {
  ...
  _createClass(Square, [{
    ...
    value: function animate() {
      (0, _animejs2.default)({
        targets: this.square,
        translateY: [{ value: 10, duration: this.frequency / 2 }, { value: 0, duration: this.frequency / 2 }],
        rotate: '1turn',
...
var Grid = function () {
  function Grid(parent, numCols, numRows, folder, filetype) {
    ...
    this.playButton.addEventListener('click', function () {
      if (_this.stopPlay) {
        ...
        _this.play(_this.pauseIndex);
        _this.playButton.classList.add('off');
        _this.pause.classList.remove('off');
      }
    });
    this.pause = document.getElementById('pause');
    this.pause.addEventListener('click', function () {
      _this.stopPlay = true;
      _this.pause.classList.add('off');
      _this.playButton.classList.remove('off');
    });
  }
  ...
  _createClass(Grid, [{
    ...
    value: function play(colIndex) {
      ...
      var nextIndex = colIndex === this.numCols - 1 ? 0 : colIndex + 1;
      setTimeout(function () {
        _this2.columns[colIndex].playAll();
...
var setupRotationControls = function setupRotationControls() {
  ...
  //individual button
  var cubeControls = document.getElementById('cube-controls');
  var cube = document.getElementById('cube');

  cubeControls.addEventListener('click', function (e) {
    e.preventDefault();
    var side = e.target.id.split("-")[0];
...
var setupInstruments = function setupInstruments() {
  ...
  drums.play(0);
  snares.play(0);
  var grids = [{ instrument: bells, face: "left" }, { instrument: drums, face: "bottom" }, { instrument: snares, face: "front" }, { instrument: hats, face: "right" }, { instrument: shakers, face: "back" }, { instrument: bongos, face: "top" }];
  setupResetButtons(grids);
};

var setupResetButtons = function setupResetButtons(grids) {
  var resetAllButton = document.getElementById('reset-all');
  resetAllButton.addEventListener('click', function () {
    grids.forEach(function (grid) {
      return grid.instrument.reset();
    });
  });

  var resetFaceButton = document.getElementById('reset-face');
  var cube = document.getElementById('cube');
  resetFaceButton.addEventListener('click', function () {
    var currentFace = cube.className.split('-')[1];
